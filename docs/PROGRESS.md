...
### Phase 2 — Analyse de perf (point 7, session du 10/06/2026)
- **Ground truth d'abord** (profilage fixture côté ANALYSE) : 52 exos force, distribution séances ≥2/≥3/≥5 = **26/18/8**, reps très étalées (Chest Dip 4-30), 4 cardio (Cycling/Walking/Running/Rowing), bodyweight FR (W=0) du jour mobilité. Validité e1RM (REP_CAP 12, MIN 3) → **16 exos suivis (tous e1RM), 0 volume naturel, 32 insuffisants, 4 cardio**. Seuils calés sur cette réalité.
- `src/lib/perf.js` (**réellement pur** — aucun `import db` ; la lecture `db.sets.toArray()` vit dans l'écran Perf) : `epley` (single → charge brute), `isValidE1rmSet`/`sessionE1rm` (top-set = max e1RM des valides ≤12 reps), `sessionTopWeight` (charge échauffement), `sessionVolume`, `groupByExercise` (séance = exercice+jour), **`chooseMetric` (DURCISSEMENT D23 : métrique PAR EXERCICE — e1RM si ≥3 séances valides, sinon volume étiqueté ; jamais de mélange)**, `metricSeries` (séances sans valeur exclues, hors fenêtre N), `trendVerdict` (fenêtre **N=3**, max 2 dernières vs max antérieures, bande **±2,5 %**, gate < 3 → null), `invertVerdict`/`isAssisted` (**exercices assistés : lecture inversée**), `personalRecords` (PR all-time, **min-assistance** pour les assistés), `computeWarmup` (paliers seam `WARMUP_SCHEME` 40/60/75/90, arrondi 2,5 kg, drop palier, plancher 20 kg, N/A bodyweight/cardio/assisté), `analyzeExercise`/`analyzeAll` (cardio exclu/label).
- **Contresens assisté corrigé (addendum D23)** : `Chest Dip (Assisted)`/`Pull Up (Assisted)` — le « poids » Strong = charge d'**assistance** (moins = plus fort). Vérifié sur la fixture (assistance 86→39, 69→53 = progression). Verdict **lu inversé** (assistance ↓ → ↑ progresse), PR = min-assistance, échauffement N/A, étiquette UI « assisté — charge = assistance ». Métrique e1RM **inchangée** → interdit de mélange D23 intact.
- `src/data/exerciseVariants.js` : table **statique** exercice → variantes anti-plateau (donnée de référence, lookup trim+casse, `[]` si inconnu). **PAS d'IA** (Phase 3).
- `src/screens/Perf.jsx` : **segmented `Synthèse | Import`** — vue Import/log D22 **intacte** (déplacée sous l'onglet) ; Synthèse = exos suivis (verdict ↑/→/↓ + valeur + badge PR) + section repliée « Données insuffisantes (N) » ; détail exo = **sparkline SVG inline** (zéro dép — contourne la dette Recharts) + historique + échauffement calculé + variantes.
- Décision actée : **D23**. **Aucune décision D1→D22 rouverte** (zéro bump Dexie, schéma D22 réutilisé, D21 `trained` non touché). 2 tests smoke antérieurs (import Strong) adaptés au segmented control (clic « Import » avant l'input).

**Vérifs** : `tests/perf.test.mjs` ✅ (Epley + plafond reps ; choix métrique sur fixture **16/32/4** ; gate insuffisance Pec Deck 2 séances → pas de verdict ; **test ANTI-MÉLANGE synthétique** haut-reps → volume étiqueté, valeur courante = volume jamais e1RM ; verdict ±2,5 % + **séance molle isolée ≠ régression** ; PR all-time ; échauffement arrondi/drop/plancher ; variantes connu/trim/inconnu ; **bloc M assistés** : détection trim+casse, `invertVerdict`, Chest Dip réel verdict brut down→lu up, PR min-assistance, échauffement N/A) · **383 PASS, 0 FAIL, 12 suites node** · smoke Playwright **15/15** ✅ — **analyse de perf** (import fixture → Synthèse → Chest Dip **« Progresse »** [assisté, lecture inversée] + label « assisté — charge = assistance » + échauffement « Sans objet » + variante « Chest Press (Machine) » + section « Données insuffisantes (32) » + log import accessible) ; 14 antérieurs verts (import Strong adapté au segmented) · `npm run lint`/`build` ✅. **Commit feat(perf) : _en attente du GO_.**
...
## En cours
- (rien) — **Phase 2 analyse de perf (D23)** codée + vérifiée (lint/build/**12 suites node = 373 PASS**/15 smoke verts), **commit feat(perf) en attente du GO**. (Import Strong D22 déjà committé/poussé `5ab972b`.)

## PROCHAINE ACTION CONCRÈTE
> **Phase 3 — Assistance** (point 8 ROADMAP) : mini-chat repas, moteur de règles 100 % local (macros restantes + contexte sport/heure → 2-3 idées de repas maison dans le budget). **À part / déférés** : boissons (~38 alcoolisées, table `drinks` prête, `sourceType:'drink'`) ; **replace-on-conflict** import Strong (D22) ; **timing horaire des glucides** autour de la séance (`startedAt` capté, porte ouverte) ; couche IA des variantes anti-plateau (D23, Phase 3) ; lazy-load Recharts (dette technique, écran Poids).
...
9. ~~**Phase 2 — intelligence glucidique (D21)**~~ ✅ (8bc8026) puis ~~**import CSV Strong (D22)**~~ ✅ (5ab972b, poussé) puis ~~**analyse de perf (D23)**~~ ✅ (commit en attente). **PHASE 2 BOUCLÉE.** Reste : **Phase 3 assistance** (point 8). _+ boissons déférées (sous-tâche)._
