...
test('nutrition : seed parti via flag, plat pesé → kcal/macros corrects → reload → entrée conservée → consommé du Jour allumé', async ({ page }) => {
  ...
  // Enregistre → navigue vers l'onglet Journal (promu en nav, D26). On attend d'abord
  // le démontage du composeur (le <select> contient un <option> homonyme du repas).
...
test('recettes : composer poulet+riz → enregistrer comme recette → rappeler → 2 entrées figées au journal → reload persiste', async ({ page }) => {
  ...
  // Journal = onglet nav principal (D26) → un seul clic, plus de détour par Bouffe.
  await page.reload()
  await page.getByRole('button', { name: 'Journal' }).click()
  await expect(page.getByText(/200 g · 240 kcal/)).toBeVisible()
  await expect(page.getByText(/150 g · 195 kcal/)).toBeVisible()
...
test('une pesée saisie est persistée et relue après reload', async ({ page }) => {
  ...
  // Et le dashboard Jour reflète la pesée du jour. `exact` OBLIGATOIRE : depuis D26
  // l'onglet « Journal » est en nav permanente → 'Jour' ⊂ 'Journal' (sans exact, 2 matches).
  await page.getByRole('button', { name: 'Jour', exact: true }).click()
...
test('nav + lien kcal (D26) : Journal promu en nav rend les entrées du jour ; « X mangé » du héro → onglet Journal', async ({ page }) => {
  const errors = []
  page.on('console', (m) => m.type() === 'error' && errors.push(m.text()))
  page.on('pageerror', (e) => errors.push(String(e)))
  ...
  // Log un repas (240 kcal) via Bouffe → onSaved navigue vers l'onglet Journal (D26).
  await page.getByRole('button', { name: 'Bouffe' }).click()
  await page.getByLabel('Choisir un ingrédient').selectOption('blanc-poulet-cru')
  await page.getByLabel('Grammes').fill('200')
  await page.getByLabel('Ajouter au plat').click()
  await page.getByRole('button', { name: 'Enregistrer le repas' }).click()
  // L'écran Journal (nav) s'est monté et liste l'entrée du jour.
  await expect(page.getByText('Repas du jour')).toBeVisible()
  await expect(page.getByText('Blanc de poulet (cru)')).toBeVisible()

  // Retour au Jour : « 240 mangé » est le SEUL élément cliquable (aria-label), tap → Journal.
  await page.getByRole('button', { name: 'Jour', exact: true }).click()
  await expect(page.getByText('240 mangé')).toBeVisible()
  await page.getByRole('button', { name: 'Voir le détail de la journée' }).click()
  await expect(page.getByText('Repas du jour')).toBeVisible()
  await expect(page.getByText(/200 g · 240 kcal/)).toBeVisible()
...
test('anneaux (D26) : JOUR sans donnée → « non saisi » (jamais 0 %) ; repas logué → ring rempli ; SEMAINE moyenne sur les SEULS jours saisis', async ({ page }) => {
  const errors = []
  page.on('console', (m) => m.type() === 'error' && errors.push(m.text()))
  page.on('pageerror', (e) => errors.push(String(e)))
  ...
  // Jour vierge : les anneaux affichent « non saisi », JAMAIS « 0 % » (honnêteté D26).
  const rings = page.getByTestId('dashboard-rings')
  await expect(rings).toBeVisible()
  await expect(rings.getByText('non saisi').first()).toBeVisible()

  // Log un repas (240 kcal).
  await page.getByRole('button', { name: 'Bouffe' }).click()
  await page.getByLabel('Choisir un ingrédient').selectOption('blanc-poulet-cru')
  await page.getByLabel('Grammes').fill('200')
  await page.getByLabel('Ajouter au plat').click()
  ...
  // JOUR : l'anneau Énergie est désormais renseigné (240 / cible).
  await expect(rings.getByText(/240 \/ 2483 kcal/)).toBeVisible()

  // SEMAINE : moyenne sur les SEULS jours saisis = 1 (les 6 autres jours vides sont
  // EXCLUS, jamais comptés 0 — sinon ce serait « 7 j saisis » et une moyenne effondrée).
  await rings.getByRole('button', { name: 'Période semaine' }).click()
  await expect(rings.getByText(/moy\. 240 kcal\/j/)).toBeVisible()
  await expect(rings.getByText('1 j saisis').first()).toBeVisible()
  await expect(rings.getByText('7 j saisis')).toHaveCount(0)
